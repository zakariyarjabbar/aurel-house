...
Images are preoptimized local WebP in three widths; `picture` selects them with an unoptimized Next Image fallback. Fonts are served locally. Optional `NEXT_PUBLIC_SITE_URL` is a build-time public origin for canonical/social URLs; no environment variable is required. Default metadata is noindex/nofollow, with no LocalBusiness or fabricated rating data.
